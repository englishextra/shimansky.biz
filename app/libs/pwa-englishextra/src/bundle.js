...
var globalRoot = "undefined" !== typeof window ? window : undefined;
/*!
...
(function (root) {
	...
	if (!root.console) {
		root.console = {};
	}var con = root.console;var prop, method;var dummy = function dummy() {};var properties = ["memory"];var methods = ("assert,clear,count,debug,dir,dirxml,error,exception,group," + "groupCollapsed,groupEnd,info,log,markTimeline,profile,profiles,profileEnd," + "show,table,time,timeEnd,timeline,timelineEnd,timeStamp,trace,warn").split(",");while (prop = properties.pop()) {
		if (!con[prop]) {
			con[prop] = {};
		}
...
(function (root) {
	...
	var blockregex = /\{\{(([@!]?)(.+?))\}\}(([\s\S]+?)(\{\{:\1\}\}([\s\S]+?))?)\{\{\/\1\}\}/g,
	    valregex = /\{\{([=%])(.+?)\}\}/g;var t = function t(template) {
		this.t = template;
	};function scrub(val) {
...
(function (root) {
	...
	var d = document,
	    qS = "querySelector",
	    aEL = "addEventListener";var Carousel = function Carousel(setting) {
...
(function (root) {
	...
	var d = document,
	    ...
	    isShowingClass = "is-showing";var IframeLightbox = function IframeLightbox(elem, rate) {
		if (elem.nodeName) {
		...
		var _this = this;if (!this.el) {
			...
			var timeout, args, context, timestamp;return function () {
				context = this;args = [].slice.call(arguments, 0);timestamp = new Date();var later = function later() {
		...
		var _this = this,
			...
			_this.close();
		});var clearBody = function clearBody(e) {
			if (_this.isOpen()) {
				return;
...
(function (root) {
	...
	var scroll2Top = function scroll2Top(scrollTargetY, speed, easing) {
		var scrollY = root.scrollY || document.documentElement.scrollTop;scrollTargetY = scrollTargetY || 0;speed = speed || 2000;easing = easing || 'easeOutSine';var currentTime = 0;var time = Math.max(0.1, Math.min(Math.abs(scrollY - scrollTargetY) / speed, 0.8));var easingEquations = { easeOutSine: function easeOutSine(pos) {
 ...
 */
(function (root) {
	var Timers = function Timers(ids) {
		this.ids = ids || [];
...
(function (root) {
	"use strict";
	var safelyParseJSON = function safelyParseJSON(a) {
		var isJson = function isJson(obj) {
			var t = typeof obj === "undefined" ? "undefined" : _typeof(obj);return ['boolean', 'number', "string", 'symbol', "function"].indexOf(t) === -1;
		};if (!isJson(a)) {
			return JSON.parse(a);
		} else {
			return a;
...
(function (root) {
	...
	root.forEach = function (arr, eachFn, doneFn) {
		var i = -1;var len = function (val) {
			...
			var _async;var abort = result === false;do {
				++i;
			} while (!(i in arr) && i !== len);if (abort || i === len) {
				if (doneFn) {
					doneFn(!abort, arr);
				}return;
			}result = eachFn.call({ async: function async() {
					_async = true;return next;
				} }, arr[i], i, arr);if (!_async) {
				next(result);
			}
		})();
	};
 ...
 */
(function (root) {
	var alignToMasterBottomLeft = function alignToMasterBottomLeft(masterId, servantId, sameWidth) {
...
(function (root) {
	"use strict";
	var insertTextAsFragment = function insertTextAsFragment(text, container, callback) {
		var d = document,
		    b = d.body || "",
		    aC = "appendChild",
		    iH = "innerHTML",
...
(function (root) {
	...
	var parseLink = function parseLink(url, full) {
		full = full || !1;return function () {
			var _r = function _r(s) {
				return s.replace(/^(#|\?)/, "").replace(/\:$/, "");
			},
			    l = location || "",
			    _p = function _p(protocol) {
...
(function (root) {
	"use strict";
	var scriptIsLoaded = function scriptIsLoaded(s) {
...
(function (root) {
	"use strict";
	var loadUnparsedJSON = function loadUnparsedJSON(url, callback, onerror) {
...
(function (root) {
	"use strict";
	var insertExternalHTML = function insertExternalHTML(id, url, callback, onerror) {
		var d = document,
		    b = d.body || "",
		    gEBI = "getElementById",
		    cN = "cloneNode",
...
var LoadingSpinner = function () {
	...
	return {
		...
		hide: function hide(callback, delay) {
			delay = delay || 500;
			var timers = new Timers();
			timers.timeout(function () {
				timers.clear();
 ...
 */
var renderTemplate = function renderTemplate(parsedJson, templateId, targetId) {
	"use strict";
 ...
 */
var insertFromTemplate = function insertFromTemplate(parsedJson, templateId, targetId, callback, useInner) {
	"use strict";
 ...
 */
var handleDataSrcImages = function handleDataSrcImages() {
	"use strict";
	...
	var d = document,
	    ...
	    rerenderDataSrcImage = function rerenderDataSrcImage(e) {
	    ...
	    arrangeDataSrcImage = function arrangeDataSrcImage(e) {
		/*!
   ...
   */
	    ...
	    rerenderDataSrcImages = function rerenderDataSrcImages() {
    ...
    handleDataSrcImagesWindow = function handleDataSrcImagesWindow() {
	var throttleHandleDataSrcImages = throttle(handleDataSrcImages, 100);
	throttleHandleDataSrcImages();
},
    manageDataSrcImages = function manageDataSrcImages() {
	"use strict";
 ...
 */
var handleDataSrcIframes = function handleDataSrcIframes() {
	"use strict";
	...
	var d = document,
	    ...
	    isBindedClass = "is-binded",
	    rerenderDataSrcIframe = function rerenderDataSrcIframe(e) {
		if (!e[cL].contains(isBindedClass)) {
			var _src = e[ds].src || "";
			if (_src) {
				if (parseLink(_src).isAbsolute && !parseLink(_src).hasHTTP) {
	    ...
	    arrangeDataSrcIframe = function arrangeDataSrcIframe(e) {
		/*!
   * true if elem is in same y-axis as the viewport or within 100px of it
   * @see {@link https://github.com/ryanve/verge}
   */
	    ...
	    rerenderDataSrcIframes = function rerenderDataSrcIframes() {
    ...
    handleDataSrcIframesWindow = function handleDataSrcIframesWindow() {
	var throttleHandleDataSrcIframes = throttle(handleDataSrcIframes, 100);
	throttleHandleDataSrcIframes();
},
    manageDataSrcIframes = function manageDataSrcIframes(ctx) {
	"use strict";
 ...
 */
var manageIframeLightboxLinks = function manageIframeLightboxLinks(ctx) {
	"use strict";
	...
	var d = document,
	    ...
	    rerenderIframeLightboxLinks = function rerenderIframeLightboxLinks() {
...
var handleExternalLink = function handleExternalLink(url, ev) {
    ...
    manageExternalLinks = function manageExternalLinks(ctx) {
	"use strict";
	...
	var d = document,
	    ...
	    gA = "getAttribute",
	    isBindedClass = "is-binded",
	    arrangeExternalLink = function arrangeExternalLink(e) {
		if (!e[cL].contains(isBindedClass)) {
			var url = e[gA]("href") || "";
	    ...
	    rerenderExternalLinks = function rerenderExternalLinks() {
		for (var i = 0, l = link.length; i < l; i += 1) {
...
var hideImgLightbox = function hideImgLightbox() {
    ...
    handleImgLightboxContainer = function handleImgLightboxContainer() {
	"use strict";
    ...
    handleImgLightboxWindow = function handleImgLightboxWindow(ev) {
	"use strict";
    ...
    manageImgLightboxLinks = function manageImgLightboxLinks(ctx) {
	"use strict";
	...
	var handleImgLightboxLink = function handleImgLightboxLink(ev) {
	    ...
	    arrangeImgLightboxLink = function arrangeImgLightboxLink(e) {
		if (!e[cL].contains(isBindedClass)) {
			var _href = e[gA]("href") || "";
			if (_href) {
				if (parseLink(_href).isAbsolute && !parseLink(_href).hasHTTP) {
	    ...
	    rerenderImgLightboxLinks = function rerenderImgLightboxLinks() {
 ...
 */
var handleOtherDropdownLists = function handleOtherDropdownLists(_this) {
	"use strict";
    ...
    manageOtherDropdownLists = function manageOtherDropdownLists() {
	"use strict";
...
var manageChaptersSelect = function manageChaptersSelect() {
	...
	var w = globalRoot,
	    ...
	    isActiveClass = "is-active",
	    isDropdownClass = "is-dropdown",
	    arrangePagesSelect = function arrangePagesSelect() {
	    ...
	    rerenderChaptersSelect = function rerenderChaptersSelect() {
		arrangePagesSelect();
		var rerenderOption = function rerenderOption(option) {
			if (option) {
	    ...
	    rerenderChaptersList = function rerenderChaptersList() {
		var handleChaptersListItem = function handleChaptersListItem(listObj, _hash) {
		    ...
		    insertChevronDownSmallSvg = function insertChevronDownSmallSvg(targetObj) {
		    ...
		    df = d[cDF](),
		    generateChaptersListItems = function generateChaptersListItems(_this, i) {
			if (0 === i) {
		...
		var handleChaptersListItemsButton = function handleChaptersListItemsButton(ev) {
			ev.stopPropagation();
			ev.preventDefault();
 ...
 * @param {Object} [ctx] context HTML Element
 */
var manageExpandingLayers = function manageExpandingLayers(ctx) {
	"use strict";
	...
	var d = document,
	    ...
	    isBindedClass = "is-binded",
	    isActiveClass = "is-active",
	    handleExpandingLayers = function handleExpandingLayers(_this) {
	    ...
	    arrangeExpandingLayers = function arrangeExpandingLayers(e) {
		if (!e[cL].contains(isBindedClass)) {
			e[aEL]("click", handleExpandingLayers);
			e[cL].add(isBindedClass);
		}
	},
	    rerenderExpandingLayers = function rerenderExpandingLayers() {
...
var msnry,
    pckry,
    initMasonry = function initMasonry(ctx) {
	"use strict";
	...
	var w = globalRoot,
	    ...
	    arrangeMasonryItems = function arrangeMasonryItems() {
		var timers;
		if (w.Masonry) {
			if (msnry) {
				msnry.destroy();
 ...
 */
var manageDisqusButton = function manageDisqusButton(ctx) {
	"use strict";
	...
	if (disqusThread && btn && disqusShortname && locationHref) {
		...
		var hideDisqusButton = function hideDisqusButton() {
		    ...
		    hideDisqusThread = function hideDisqusThread() {
		    ...
		    renderDisqusThread = function renderDisqusThread() {
			try {
				DISQUS.reset({
					reload: !0,
		    ...
		    handleDisqusButton = function handleDisqusButton(ev) {
			ev.stopPropagation();
			ev.preventDefault();
			var logicHandleDisqusButton = function logicHandleDisqusButton() {
...
var notiBar = function notiBar(opt) {
	...
	if (b) {
		...
		var set_cookie = function set_cookie() {
			if (settings.days) {
...
var manageSearchInput = function manageSearchInput() {
	...
	var d = document,
	    ...
	    handleSearchInputValue = function handleSearchInputValue() {
		var _this = this;
		var logicHandleSearchInputValue = function logicHandleSearchInputValue() {
...
var initKamilAutocomplete = function initKamilAutocomplete(jsonObj) {
	...
	var w = globalRoot,
	    ...
	    generateMenu = function generateMenu(jsonResponse) {
		if (jsonResponse) {
    ...
    */
			var typoAutcompleteList = d[cE]("ul"),
			    ...
			    handleTypoSuggestions = function handleTypoSuggestions() {
				typoAutcompleteList.style.display = "none";
				typoListItem.style.display = "none";
			},
			    showTypoSuggestions = function showTypoSuggestions() {
    ...
    */
     ...
     * limit output
     */
				limitKamilOutput = function limitKamilOutput(e, i) {
					if (i < 10) {
     ...
     */
				var logicReplaceTypo = function logicReplaceTypo() {
					while (itemsLength < 1) {
     ...
     */
				var lis = ul ? ul[gEBTN]("li") || "" : "",
				    truncateKamilText = function truncateKamilText(e) {
    ...
    */
			ac.on("kamilselect", function (e) {
				var kamilPath = e.item.href || "",
				    triggerOnKamilSelect = function triggerOnKamilSelect() {
...
var renderNavigation = function renderNavigation() {
	...
	var w = globalRoot,
	    ...
	    isActiveClass = "is-active",
	    handleListItems = function handleListItems(e) {
	    ...
	    alignNavbarLists = function alignNavbarLists() {
	    ...
	    handleShowRenderNavbarPopularButton = function handleShowRenderNavbarPopularButton(ev) {
		ev.stopPropagation();
		ev.preventDefault();
		renderNavbarPopular[cL].toggle(isActiveClass);
		handleOtherDropdownLists(renderNavbarPopular);
	},
	    handleShowRenderNavbarMoreButton = function handleShowRenderNavbarMoreButton(ev) {
		ev.stopPropagation();
		ev.preventDefault();
		renderNavbarMore[cL].toggle(isActiveClass);
		handleOtherDropdownLists(renderNavbarMore);
	},
	    handleShowNavbarListsWindow = function handleShowNavbarListsWindow() {
	...
	if (navbar) {
		var processNavigationJsonResponse = function processNavigationJsonResponse(navigationJsonResponse) {
			...
			if (carouselTemplate && carouselRender) {
				insertFromTemplate(navigationJsonResponse, carouselTemplateId, carouselRenderId, function () {
					var newCarousel = function newCarousel() {
						return new Carousel({
							"main": ".js-carousel",
...
var fixUiPanelContentsSelect = function fixUiPanelContentsSelect() {
	...
	var w = globalRoot,
	    ...
	    isFixedClass = "is-fixed",
	    handleUiPanelContentsSelect = function handleUiPanelContentsSelect() {
		var logicHandleUiPanelContentsSelect = function logicHandleUiPanelContentsSelect() {
 ...
 * @param {Object} [_this] node element, if empty will affect everyone
 */
var handleOtherSocialButtons = function handleOtherSocialButtons(_this) {
	"use strict";
    ...
    manageOtherSocialButtons = function manageOtherSocialButtons() {
	"use strict";
	...
	if (btn && holder && locationHash) {
		if ("undefined" !== typeof getHTTP && getHTTP()) {
			...
			var generateLocationQrCodeImg = function generateLocationQrCodeImg() {
				...
				newImg.alt = newTitle;
				var renderNewQrCode = function renderNewQrCode() {
					if (w.QRCode) {
			    ...
			    handleLocationQrCodeButton = function handleLocationQrCodeButton(ev) {
				ev.stopPropagation();
				ev.preventDefault();
...
var manageDebugGridButton = function manageDebugGridButton() {
	...
	var w = globalRoot,
	    ...
	    cookieDatum = "ok",
	    handleDebugGridContainer = function handleDebugGridContainer() {
		if (container) {
			container[cL].remove(debugClass);
			container[rEL]("click", handleDebugGridContainer);
		}
	},
	    showDebugGridMessage = function showDebugGridMessage() {
		var col = d[qS](".col") || "",
		    elements = [b, page, container, col],
		    debugMessage = [],
		    renderElementsInfo = function renderElementsInfo(e) {
			if (e) {
 ...
 */
var processPoutes = function processPoutes() {
	"use strict";
	...
	if (appContent) {
		var contentsListButtonDefaultText = contentsSelect ? contentsSelect.options[0].firstChild.textContent || "" : "",
		    ...
		    processRoutesJsonResponse = function processRoutesJsonResponse(routesJsonResponse) {
			var routesParsedJson = JSON.parse(routesJsonResponse),
			    renderMasonry,
			    renderComments,
			    triggerOnContentInserted = function triggerOnContentInserted(titleString, nextHrefString, asideObj, routesObj) {
     ...
     */
     ...
     contentsRender[iH] = contentsRendered; */
					var rerenderContentsSelect = function rerenderContentsSelect() {
						var handleContentsSelect = function handleContentsSelect(_this) {
							_this = _this ? _this.target || "" : "";
							var _hash = _this.options[_this.selectedIndex].value || "";
							if (_hash) {
						    ...
						    df = d[cDF](),
						    generateContentsOptions = function generateContentsOptions(e) {
							if (e.title) {
					...
					var rerenderContentsList = function rerenderContentsList() {
						var handleContentsListItem = function handleContentsListItem(listObj, _hash) {
							if (_hash) {
								var tragetObject = isValidId(_hash, true) ? d[gEBI](_hash.replace(/^#/, "")) || "" : "";
								if (tragetObject) {
									scroll2Top(findPos(tragetObject).top, 10000);
						    ...
						    df = d[cDF](),
						    generateContentsListItems = function generateContentsListItems(e) {
							if (e.title) {
						...
						var handleContentsListItemsButton = function handleContentsListItemsButton(ev) {
							ev.stopPropagation();
							ev.preventDefault();
...
var initUiTotop = function initUiTotop() {
	...
	var w = globalRoot,
	    ...
	    btnTitle = "Наверх",
	    isActiveClass = "is-active",
	    handleUiTotopWindow = function handleUiTotopWindow(_this) {
	    ...
	    renderUiTotop = function renderUiTotop() {
		var handleUiTotopAnchor = function handleUiTotopAnchor(ev) {
			ev.stopPropagation();
			ev.preventDefault();
			scroll2Top(0, 20000);
		},
		    insertUpSvg = function insertUpSvg(targetObj) {
			var svg = d[cENS]("http://www.w3.org/2000/svg", "svg"),
			    use = d[cENS]("http://www.w3.org/2000/svg", "use");
			svg[cL].add("ui-icon");
 ...
 * init manUP.js
 */
var loadInitManUp = function loadInitManUp() {
	"use strict";

	var manUpJsUrl = "/cdn/ManUp.js/0.7/js/manup.fixed.min.js",
	    initManUp = function initManUp() {
 ...
 */
var showPageFinishProgress = function showPageFinishProgress() {
	"use strict";
	...
	var d = document,
	    ...
	    triggerOnImagesLoaded = function triggerOnImagesLoaded() {
		var timers = new Timers();
		timers.interval(function () {
